...
unsigned int ttloadTexture(const char *path, bool gammaCorrection);
void renderQuad();
void renderCube();
...
const glm::vec3 LIGHT_COLOR = glm::vec3(0.2, 0.3, 0.4); // CHANGE AREA LIGHT COLOR HERE!
bool keys[1024];                                        // activated keys
glm::vec3 areaLightTranslate;
GLSingleShader *ltcShaderPtr;

// camera
GLBasicCamera camera(glm::vec3(0.0f, 1.0f, 0.5f), glm::vec3(0.0f, 1.0f, 0.0f), 180.0f, 0.0f);
...
void incrementRoughness(float step)
{
    ...
    ltcShaderPtr->use();
    ltcShaderPtr->setUniform("material.albedoRoughness", glm::vec4(color, roughness));
    glUseProgram(0);
}
...
void incrementLightIntensity(float step)
{
    ...
    ltcShaderPtr->use();
    ltcShaderPtr->setUniform("areaLight.intensity", intensity);
    glUseProgram(0);
}
...
void switchTwoSided(bool doSwitch)
{
    ...
    ltcShaderPtr->use();
    ltcShaderPtr->setUniform("areaLight.twoSided", twoSided);
    glUseProgram(0);
}
...
int main()
{
    ...
    // SHADERS
    GLSingleShader shaderLTC("resource/shader/arealight/arealight");
    ltcShaderPtr = &shaderLTC;
    GLSingleShader shaderLightPlane("resource/shader/arealight/light_plane");

    // TEXTURES
    unsigned int concreteTexture = autoLoadTexture("resource/img/concreteTexture.png"); // ttloadTexture("resource/img/concreteTexture.png", true);

    // SHADER CONFIGURATION
    shaderLTC.use();
    shaderLTC.setUniform("areaLight.points[0]", areaLightVertices[0].position);
    shaderLTC.setUniform("areaLight.points[1]", areaLightVertices[1].position);
    shaderLTC.setUniform("areaLight.points[2]", areaLightVertices[4].position);
    shaderLTC.setUniform("areaLight.points[3]", areaLightVertices[5].position);
    shaderLTC.setUniform("areaLight.color", LIGHT_COLOR);
    shaderLTC.setUniform("LTC1", 0);
    shaderLTC.setUniform("LTC2", 1);
    shaderLTC.setUniform("material.diffuse", 2);
    ...
    {
        glm::mat4 model(1.0f);
        shaderLightPlane.setUniform("model", model);
    }
    shaderLightPlane.setUniform("lightColor", LIGHT_COLOR);
    glUseProgram(0);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        shaderLTC.setUniform("model", model);
        shaderLTC.setUniform("normalMatrix", normalMatrix);
        glm::mat4 view = camera.viewMatrix();
        shaderLTC.setUniform("view", view);
        glm::mat4 projection = camera.projectionMatrix(SCR_WIDTH, SCR_HEIGHT);
        shaderLTC.setUniform("projection", projection);
        shaderLTC.setUniform("viewPosition", camera.positionVector());
        shaderLTC.setUniform("areaLightTranslate", areaLightTranslate);
        ...
        shaderLightPlane.setUniform("model", model);
        shaderLightPlane.setUniform("view", view);
        shaderLightPlane.setUniform("projection", projection);
        renderAreaLight();
        glUseProgram(0);
...
void do_movement(GLfloat deltaTime)
{
    ...
    if (keys[GLFW_KEY_W])
    {
        camera.processDirection(CameraMovement::FORWARD, cameraSpeed);
    }
    else if (keys[GLFW_KEY_S])
    {
        camera.processDirection(CameraMovement::BACKWARD, cameraSpeed);
    }
    if (keys[GLFW_KEY_A])
    {
        camera.processDirection(CameraMovement::LEFT, cameraSpeed);
    }
    else if (keys[GLFW_KEY_D])
    {
        camera.processDirection(CameraMovement::RIGHT, cameraSpeed);
    }
...
// ---------------------------------------------------
unsigned int ttloadTexture(char const *path, bool gammaCorrection)
{

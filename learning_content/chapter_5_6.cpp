...
// render func
void RenderQuad();
void RenderCube(GLSingleShader &shader);
...
int main()
{
    ...
    // 加载着色器
    GLSingleShader shader("resource/shader/chapter_5/normal_mapping");
    // 加载纹理
    GLuint diffuseMap = autoLoadTexture("resource/img/brickwall.jpg");
    GLuint normalMap = autoLoadTexture("resource/img/brickwall_normal.jpg");
    // Set texture units 
    shader.use();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if(normalMapping && ImGui::Button("Disbale Normal Mapping"))
            normalMapping = !normalMapping;
        if(!normalMapping && ImGui::Button("Enable Normal Mapping"))
            normalMapping = !normalMapping;

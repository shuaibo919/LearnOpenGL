...
private:
    ...
    unsigned int m_shaderProgramIndex;
    GLuint m_VAO, m_VBO, m_shaderProgram;
    const std::string m_vshader = "Resource/Shader/line.vert";
    const std::string m_fshader = "Resource/Shader/line.frag";
...
public:
    ...
    void SetStartPoint(glm::vec3 val)
    {
        m_vetices[0] = val.x;
        m_vetices[1] = val.y;
        m_vetices[2] = val.z;
    }
    void SetEndPoint(glm::vec3 val)
    {
        m_vetices[3] = val.x;
        m_vetices[4] = val.y;
        m_vetices[5] = val.z;
    }
    void SetLineColor(glm::vec3 val)
    {
        m_line_color = val;
    }
    void BindVertex()
    {
        ...
        glBufferData(GL_ARRAY_BUFFER, sizeof(m_vetices), m_vetices, GL_STATIC_DRAW);
...
int main()
{
    ...
    if (glfwgladInitialization(&window, SRC_WIDTH, SRC_HEIGHT, "Line") == -1)
        return -1;
    glfwSwapInterval(1);
    ...
    // 加载着色器
    GLSingleShader shader("resource/shader/chapter_5/line_gl");

    LineObject line = LineObject();
    line.SetStartPoint(glm::vec3(0.f,0.,0.1));
    line.SetEndPoint(glm::vec3(1.f,1.,1.1));
    // Set texture units
    shader.use();
    shader.setUniform("model", glm::mat4(1.0f));
    shader.setUniform("color", glm::vec3(1.f,0.,0.1));
